...
//void Fl_Input_::drawtext(int X, int Y, int W, int H, int align, Fl_Image *img) {
//	int do_mu = !(damage()&FL_DAMAGE_ALL);
//
//	if (Fl::focus() != this && !size()) {
//		if (do_mu) { // we have to erase it if cursor was there
...
//	int desc = height - fl_descent();
//	float xpos;
//	int width = fl_width(value(),strlen(value()));
//	int ypos;
//	//int align = 0;
...
//
//	if (align & FL_ALIGN_LEFT) xpos = X + imageW;
//	else if (align & FL_ALIGN_RIGHT) xpos = X + W - (int)(width + imageW + 1);
//	else xpos = X + (W - (int)(width + imageW)) / 2;
//
//	for (; ypos < H;) {
//
//		// re-expand line unless it is the last one calculated above:
