...
namespace base
{
	...
	date::date(int y, int m, int d, int wd) : year(y), month(m), day(d), wday(wd)
	{

	}

	date::date(const date& date) : year(date.year), month(date.month), day(date.day), wday(date.wday)
	{
	...
	datetime& datetime::now()
	{
		...
		SYSTEMTIME sys;
		GetLocalTime(&sys);
		m_date = date(sys.wYear, sys.wMonth, sys.wDay, sys.wDayOfWeek);
