...
namespace Storage
{
	...
	vector<string> RecipeStorageService::SelectRecipeRecordByRow(string recipename,int row, int cols) {
		char sql[SQLCMDLEN] = { 0 };
		snprintf(sql, sizeof(sql), "SELECT * FROM %s WHERE 序号 == '%s';",
	...
	bool RecipeStorageService::OrderRecipeRecord(string recipename, int mode)
	{
		...
		if (mode)	//逆序
			snprintf(sql, sizeof(sql), "ISELECT * FROM %s ORDER BY 序号 DESC", recipename.c_str());
		else		//顺序
			snprintf(sql, sizeof(sql), "ISELECT * FROM %s ORDER BY 序号 ASC", recipename.c_str());
		string sqlstr = sql;
	...
	bool RecipeStorageService::AddRecipeRecord(string recipename, int row)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "INSERT INTO %s (序号) VALUES (%s)", recipename.c_str(), to_string(row).c_str());
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
			UI::IResourceService::GB2312toUtf8(sqlstr);
		char * errMsg = NULL;
	...
	bool RecipeStorageService::InsertRecipeRecord(string recipename, int row)
	{
		...
		struct sqlite3_stmt *stmt;
		/*将Id往后加*/
		snprintf(sql, sizeof(sql), "UPDATE %s SET 序号 = 序号 + '1' WHERE 序号 > '%s'", recipename.c_str(), to_string(row).c_str());
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
			UI::IResourceService::GB2312toUtf8(sqlstr);
		char * errMsg = NULL;
	...
	bool RecipeStorageService::DeleteRecipeRecord(string recipename, int row)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "DELETE FROM %s WHERE 序号 = '%s';UPDATE %s SET 序号 = 序号 - '1' WHERE 序号 > '%s'",
	...
	bool RecipeStorageService::MoveUpRecipeRecord(string recipename, int row)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "UPDATE %s SET 序号 = 't' WHERE 序号 = '%s';\
			UPDATE %s SET 序号 = '%s' WHERE 序号 = '%s';\
			UPDATE %s SET 序号 = '%s' WHERE 序号 = 't'",
			recipename.c_str(), to_string(row).c_str(), 
	...
	bool RecipeStorageService::UpdateRecipeRecord(string recipename, string colname, int rowcol, string data)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "UPDATE %s SET %s = '%s' WHERE 序号 = '%s';",
