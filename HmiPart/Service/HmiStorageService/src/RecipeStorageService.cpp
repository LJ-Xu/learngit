...
namespace Storage
{
	...
	vector<vector<string>> RecipeStorageService::SelectAllRecipeRecord(string recipename, int col) {
		...
		//OrderRecipeRecord(recipename);
		snprintf(sql, sizeof(sql), "SELECT * FROM %s ORDER BY 序号 ASC", recipename.c_str());
		string sqlstr = sql;
		...
		return std::move(SelectRecipe(sqlstr.c_str(), col));
	}
	vector<string> RecipeStorageService::SelectRecipeRecordByRow(string recipename,int row, int cols) {
		char sql[SQLCMDLEN] = { 0 };
		snprintf(sql, sizeof(sql), "SELECT * FROM %s WHERE 序号 == %d;",
			recipename.c_str(),row);
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
			UI::IResourceService::GB2312toUtf8(sqlstr);
		char * errMsg = NULL;
		//int ret = sqlite3_exec(db, sqlstr.c_str(), NULL, NULL, &errMsg);
		return std::move(SelectRowRecipe(sqlstr.c_str(), cols));
	}
	...
	bool RecipeStorageService::OrderRecipeRecord(string recipename, int mode)
	{
		...
		if (mode)	//逆序
			snprintf(sql, sizeof(sql), "SELECT * FROM %s ORDER BY 序号 DESC", recipename.c_str());
		else		//顺序
			snprintf(sql, sizeof(sql), "SELECT * FROM %s ORDER BY '序号' ASC", recipename.c_str());
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
	...
	bool RecipeStorageService::AddRecipeRecord(string recipename, int row)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "INSERT INTO %s (序号) VALUES (%d)", recipename.c_str(), row);
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
			UI::IResourceService::GB2312toUtf8(sqlstr);
		char * errMsg = NULL;
	...
	bool RecipeStorageService::InsertRecipeRecord(string recipename, int row)
	{
		...
		struct sqlite3_stmt *stmt;
		/*将Id往后加*/
		snprintf(sql, sizeof(sql), "UPDATE %s SET 序号 = 序号 + 1 WHERE 序号 > %d", recipename.c_str(), row);
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
			UI::IResourceService::GB2312toUtf8(sqlstr);
		char * errMsg = NULL;
	...
	bool RecipeStorageService::DeleteRecipeRecord(string recipename, int row)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "DELETE FROM %s WHERE 序号 = %d;UPDATE %s SET 序号 = 序号 - 1 WHERE 序号 > %d",
	...
	{
		if (InsertRecipeRecord(recipename, row))		//插入一行
		{
			char sql[SQLCMDLEN] = { 0 };
			struct sqlite3_stmt *stmt;
			...
			{
				sqlstr = sqlstr + " " + colnames[i].ColTitle + " = " + data[i + 2];
				if (i != size - 1)
					sqlstr += " ,";
			}
			sqlstr += " WHERE 序号 = " + to_string(row + 1);
			//memcpy(sql, sqlstr.c_str(), sizeof(sql));
			//string sqlstr = sql;
			if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
				UI::IResourceService::GB2312toUtf8(sqlstr);
	...
	bool RecipeStorageService::MoveUpRecipeRecord(string recipename, int row)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "UPDATE %s SET 序号 = -1 WHERE 序号 = %d;\
			UPDATE %s SET 序号 = %d WHERE 序号 = %d;\
			UPDATE %s SET 序号 = %d WHERE 序号 = -1",
			recipename.c_str(), row,
			recipename.c_str(), row, row -1,
			recipename.c_str(), row - 1);
		string sqlstr = sql;
		if (!UI::CodeFormatUtility::IsStrUtf8(sqlstr.c_str()))
			UI::IResourceService::GB2312toUtf8(sqlstr);
		char * errMsg = NULL;
	...
	bool RecipeStorageService::UpdateRecipeRecord(string recipename, string colname, int rowcol, string data)
	{
		char sql[SQLCMDLEN] = { 0 };
		struct sqlite3_stmt *stmt;
		snprintf(sql, sizeof(sql), "UPDATE %s SET %s = '%s' WHERE 序号 = %d;",
