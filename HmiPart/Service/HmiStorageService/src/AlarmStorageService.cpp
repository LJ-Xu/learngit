...
namespace Storage
{
	...
	int AlarmStorageService::UpdateAlarmRecordByConfirm(uint16_t alarmGroup, uint16_t alarmNo, const AlarmRecord & record)
	{
		char sql[SQLCMDLEN] = { 0 };
		// 更新指定报警记录
		snprintf(sql, sizeof(sql), "UPDATE Alarm SET StartTime = %lld, Title = '%s', Times = %d = %lld, CheckTime = %lld, Level = %d WHERE AlarmGroup = %d AND AlarmNo = %d; UPDATE fileDb.Alarm SET StartTime = %lld, Title = '%s', Times = %d, CheckTime = %lld, Level = %d WHERE AlarmGroup = %d AND AlarmNo = %d;",
