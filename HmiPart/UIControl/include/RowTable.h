...
namespace UI
{
	class RowTable : public TableBase {
	public:
	...
	protected:
		...
		int find_cell(TableContext context,		// find cell's x/y/w/h given r/c
			...
			return(TableBase::find_cell(context, R, C, X, Y, W, H));
		}
	...
	public:
		 ...
		 */
		RowTable(int X, int Y, int W, int H, const char *l = 0) : TableBase(X, Y, W, H, l) {
			_dragging_select = 0;
			_last_row = -1;
			_last_y = -1;
			_last_push_x = -1;
		...
		void clear() {
			...
			cols(0);
			TableBase::clear();	// clear the table
		}
	};
