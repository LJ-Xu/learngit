...
// If the array is this, we will double-reallocate as necessary:
	static DropMenuItem* local_array = 0;
	static int local_array_alloc = 0; // number allocated
	static int local_array_size = 0; // == size(local_array)
	extern DropMenuUnit* fl_menu_array_owner; // in DropMenuUnit.cxx

	// For historical reasons there are matching methods that work on a
	// user-allocated array of DropMenuItem.  These methods are quite
	// depreciated and should not be used.  These old methods use the
	// above pointers to detect if the array belongs to an DropMenuUnit
	// widget, and if so it reallocates as necessary.



	// Insert a single DropMenuItem into an array of size at offset n,
	// if this is local_array it will be reallocated if needed.
	static DropMenuItem* array_insert(
		DropMenuItem* array,  // array to modify
		...
		if (array == local_array && size >= local_array_alloc) {
			...
			DropMenuItem* newarray = new DropMenuItem[local_array_alloc];
			memmove(newarray, array, size * sizeof(DropMenuItem));
			delete[] local_array;
			local_array = array = newarray;
		}
		// move all the later items:
		memmove(array + n + 1, array + n, sizeof(DropMenuItem)*(size - n));
		// create the new item:
		DropMenuItem* m = array + n;
	...
	/** Adds a menu item.
	  ...
	  \see DropMenuItem::insert(int, const char*, int, Fl_Callback*, void*, int)
	*/
	int DropMenuItem::add(
		const char *mytext,
		int sc,
		Fl_Callback *cb,
		void *data,
	...
	/**
	 ...
	 If \p index is -1, the item is added the same way as DropMenuItem::add().
	 ...
	 \param[in] myflags	menu flags as described in DropMenuItem

	 \returns the index into the menu() array, where the entry was added
	*/
	int DropMenuItem::insert(
		int index,
		const char *mytext,
		int sc,
		Fl_Callback *cb,
		...
		for (;;) {
			...
			*q = 0;
			...
			for (; !m->text.empty(); m = m->next())
				if (m->flags&FL_SUBMENU && !compare(item, m->text.c_str())) break;

			if (m->text.empty()) { /* create a new menu */
		...
		for (; !m->text.empty(); m = m->next())
			if (!(m->flags&FL_SUBMENU) && !compare(m->text.c_str(), item)) break;

		if (m->text.empty()) {	/* add a new menu item */
			int n = (index == -1) ? (int)(m - array) : index;
	 ...
	 */
	int DropMenuUnit::add(const char *label, int shortcut, Fl_Callback *callback, void *userdata, int flags) {
	...
	*/
	...
	int DropMenuUnit::insert(
		...
		if (this != fl_menu_array_owner) {
			if (fl_menu_array_owner) {
				DropMenuUnit* o = fl_menu_array_owner;
				// the previous owner gets its own correctly-sized array:
				int value_offset = (int)(o->value_ - local_array);
				int n = local_array_size;
				DropMenuItem* newMenu = o->menu_ = new DropMenuItem[n];
				memcpy(newMenu, local_array, n * sizeof(DropMenuItem));
			...
			if (menu_) {
				...
				if (local_array) {
					...
					local_array = menu_ = new DropMenuItem[local_array_alloc];
					memset(local_array, 0, sizeof(DropMenuItem) * local_array_alloc);
				}
				memset(menu_, 0, sizeof(DropMenuItem));
				local_array_size = 1;
			}
	...
	/**
	  ...
	  \returns the index into the menu() array, where the entry was added
	*/
	int DropMenuUnit::add(const char *str) {
		char buf[1024];
		int r = 0;
		while (*str) {
			int sc = 0;
	...
	*/
	//void DropMenuUnit::replace(int i, const char *str) {
	...
	/**
	  ...
	  \param i index into menu array
	*/
	//void DropMenuUnit::remove(int i) {
	//	int n = size();
	//	if (i < 0 || i >= n) return;
	//	if (!alloc) copy(menu_);
	//	// find the next item, skipping submenus:
	//	DropMenuItem* item = menu_ + i;
	//	const DropMenuItem* next_item = item->next();
	//	// delete the text only if all items were created with add():
	//	if (alloc > 1) {
	//		for (DropMenuItem* m = item; m < next_item; m++)
	//			if (m->text) free((void*)(m->text));
	//	}
	//	// MRS: "n" is the menu size(), which includes the trailing NULL entry...
	//	memmove(item, next_item, (menu_ + n - next_item) * sizeof(DropMenuItem));
	//}
