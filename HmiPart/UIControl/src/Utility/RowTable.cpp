...
{
	// Is row selected?
	int RowTable::row_selected(int row) {
		if (row < 0 || row >= rows()) return(-1);
		return(_rowselect[row]);
	}

	// Change row selection type
	void RowTable::type(TableRowSelectMode val) {
		_selectmode = val;
		switch (_selectmode) {
		case SELECT_NONE: {
	...
	//
	int RowTable::select_row(int row, int flag) {
		int ret = 0;
		if (row < 0 || row >= rows()) { return(-1); }
		switch (_selectmode) {
		case SELECT_NONE:
	...
	void RowTable::select_all_rows(int flag) {
		switch (_selectmode) {
		case SELECT_NONE:
			return;
