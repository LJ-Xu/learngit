...
namespace UI
{
	...
	bool CodeFormatUtility::IsStrUtf8(const char* str)
	{
	...
	bool CodeFormatUtility::IsStrGbk(const char* str)
	{
...
#define RET_TOOSMALL   -2
	...
	int CodeFormatUtility::GB2312ToUtf8(char*  ptDestText,
		int  nDestLength,
		char*  ptSrcText,
		int  nSrcLength)
	{
		...
		for (i = 0;i < (nSrcLength);i++, nTemp1++) {
			...
			else if (gb2312_mbtowc(0, &unicode32_text[nTemp1], (unsigned char *)&ptSrcText[i], 2) == 2) {
				i++;
				nTemp++;
			}
	...
	int CodeFormatUtility::Utf8ToGB2312(char *gb2312_str,
		int len_gb2312,
		char *utf8_str,
		int len_utf8)
	{
	...
	bool CodeFormatUtility::CodeToUtf8(CodeRule rule, char *data, int len)
	{
		switch (rule)
		{
		...
		//}
		//break;
		case UI::CodeFormatUtility::UTF_16:			//Utf16转Utf8
		{
			UTF16 *dstdata = new UTF16[len / 2];
			memcpy(dstdata, data, len);
			UTF16ToUTF8(dstdata, dstdata + len / 2, (UTF8*)data, (UTF8*)data + len);
			delete[] dstdata;
			return true;
		}
		}
	...
	bool CodeFormatUtility::Utf8ToCode(CodeRule rule, char *data, int len)
	{
		...
		case UI::CodeFormatUtility::ASCII:
		{
			int dstlen = fl_utf8toa(data, len, NULL, 0) + 1;
			char *dstdata = new char[dstlen];
			fl_utf8toa(data, len, dstdata, dstlen);
			memcpy(data, dstdata, len);
			delete dstdata;
		}
		break;
		case UI::CodeFormatUtility::UTF_16:			//Utf16转Utf8
		{
			UTF16 *dstdata = new UTF16[len / 2 + 1];
			UTF8ToUTF16((UTF8*)data, (UTF8*)data + len, dstdata, dstdata + len / 2 + 1);
			memcpy(data, dstdata, len);
			delete[] dstdata;
			break;
		}
		case UI::CodeFormatUtility::GB2312:
		{
			...
			int rlen = 0;
			rlen = MultiByteToWideChar(CP_UTF8, 0, data, -1, NULL, NULL);
			pUnicodeBuff = new WCHAR[rlen + 1];  //为Unicode字符串空间
			rlen = MultiByteToWideChar(CP_UTF8, 0, data, -1, pUnicodeBuff, rlen);
			rlen = WideCharToMultiByte(936, 0, pUnicodeBuff, -1, NULL, NULL, NULL, NULL); //936 为windows gb2312代码页码
			WideCharToMultiByte(936, 0, pUnicodeBuff, -1, dst, rlen, NULL, NULL);
			memcpy(data, dst, len);
			delete[] pUnicodeBuff;
#else
			iconv_t  cd;
	...
	std::string CodeFormatUtility::CodeConversion(std::string src, CodeRule rule)
	{
		char dst[1024] = {0};
		int srclen = src.size();
		srclen = strlen(src.c_str());
		if(!IsStrUtf8(src.c_str()))
			UI::IResourceService::GB2312toUtf8(src);
		switch (rule)
		{
		case UI::CodeFormatUtility::ASCII:
		{
			int dstlen = fl_utf8toa(src.c_str(), srclen, NULL, 0) + 1;
			fl_utf8toa(src.c_str(), srclen, dst, dstlen);
		}
			break;
		case UI::CodeFormatUtility::UTF_8:
			memcpy(dst, src.c_str(), src.size());
			break;
		case UI::CodeFormatUtility::UTF_16:
		{
			int dstlen = fl_utf8toUtf16(src.c_str(), srclen, NULL, 0) + 1;
			fl_utf8toUtf16(src.c_str(), srclen, (unsigned short *)dst, dstlen);
			break;
		}
		case UI::CodeFormatUtility::GB2312:
		{
			...
			wchar_t * pUnicodeBuff = NULL;
			int rlen = 0;
			rlen = MultiByteToWideChar(CP_UTF8, 0, src.c_str(), -1, NULL, NULL);
			pUnicodeBuff = new WCHAR[rlen + 1];  //为Unicode字符串空间
			rlen = MultiByteToWideChar(CP_UTF8, 0, src.c_str(), -1, pUnicodeBuff, rlen);
			rlen = WideCharToMultiByte(936, 0, pUnicodeBuff, -1, NULL, NULL, NULL, NULL); //936 为windows gb2312代码页码
			WideCharToMultiByte(936, 0, pUnicodeBuff, -1, dst, rlen, NULL, NULL);
			delete[] pUnicodeBuff;
#else
			char srcutf8[1024] = { 0 };
            char *ps = srcutf8;
			memcpy(srcutf8, src.c_str(), src.size());
			iconv_t  cd;
			size_t ret;
			cd = iconv_open("GBK", "UTF-8");
			if (cd == (iconv_t)-1) {
	...
	std::string CodeFormatUtility::EndianSwap(std::string srcstr)
	{
		if (srcstr.size() % 2)
			srcstr += ' ';
		//UINT8* pData = new UINT8[srcstr.size()];
		UINT8* pData = (UINT8*)(srcstr.data());
		for (size_t i = 0; i < srcstr.size(); i += 2)
		{
			int j, cnt, end, start;
			cnt = 2 / 2;
			start = i;
			...
			{
				tmp = pData[start + j];
				pData[start + j] = pData[end - j];
				pData[end - j] = tmp;
			}
		}
		/*int end = srcstr.size() - 1;
		if (srcstr.size() % 2)
		{
			if(strlen((char *)pData) > end)
				pData[end] = srcstr[end];
		}
			*/
		return (char*)pData;
	}
