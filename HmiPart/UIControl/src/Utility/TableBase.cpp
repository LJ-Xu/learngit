...
namespace UI
{
	void TableBase::row_position(int row) {
	...
	void TableBase::col_position(int col) {
	...
	long TableBase::row_scroll_position(int row) {
		int startrow = 0;
		long scroll = 0;
		// OPTIMIZATION: 
	...
	long TableBase::col_scroll_position(int col) {
		int startcol = 0;
		long scroll = 0;
		// OPTIMIZATION: 
		//     Attempt to use precomputed row scroll position
	...
	// Ctor
	TableBase::TableBase(int X, int Y, int W, int H, const char *l) : Fl_Group(X, Y, W, H, l) {
		_rows = 0;
		_cols = 0;
		_row_header_w = 40;
		_col_header_h = 18;
	...
	int TableBase::row_col_clamp(TableContext context, int &R, int &C) {
		int clamped = 0;
	...
	void TableBase::get_bounds(TableContext context, int &X, int &Y, int &W, int &H) {
		switch (context) {
		case CONTEXT_COL_HEADER:
	...
	//
	TableBase::TableContext TableBase::cursor2rowcol(int &R, int &C, ResizeFlag &resizeflag) {
		// return values
		R = C = 0;
	...
	int TableBase::find_cell(TableContext context, int R, int C, int &X, int &Y, int &W, int &H) {
		...
		default:
			fprintf(stderr, "TableBase::find_cell: unknown context %d\n", (int)context);
			return(-1);
		}
		//NOTREACHED
	}
	...
	void TableBase::_start_auto_drag() {
	...
	void TableBase::_stop_auto_drag() {
		if (!_auto_drag) return;
		Fl::remove_timeout(_auto_drag_cb2, this);
		_auto_drag = 0;
	}

	void TableBase::_auto_drag_cb2(void *d) {
		((TableBase*)d)->_auto_drag_cb();
	}

	// Handle automatic scroll-selection if mouse selection dragged off table edge
	void TableBase::_auto_drag_cb() {
		int lx = Fl::e_x;
		int ly = Fl::e_y;
	...
	void TableBase::recalc_dimensions() {
	...
	void TableBase::scroll_cb(Fl_Widget*w, void *data) {
		TableBase *o = (TableBase*)data;
	...
	void TableBase::change_cursor(Fl_Cursor newcursor) {
	...
	void TableBase::damage_zone(int r1, int c1, int r2, int c2, int r3, int c3) {
		int R1 = r1, C1 = c1;
		int R2 = r2, C2 = c2;
		if (r1 > R2) R2 = r1;
		if (r2 < R1) R1 = r2;
	...
	int TableBase::move_cursor(int R, int C, int shiftselect) {
		if (select_row == -1) R++;
		if (select_col == -1) C++;
		R += select_row;
		C += select_col;
	...
	int TableBase::move_cursor(int R, int C) {
...
// Handle FLTK events
	int TableBase::handle(int event) {
		PRINTEVENT;
		...
		case FL_KEYBOARD: {
			...
			case FL_Tab:
				...
				break;
#else
				break;				// without tab_cell_nav(), TableBase should default to navigating widgets, not cells
#endif
			}
	...
	//
	void TableBase::resize(int X, int Y, int W, int H) {
	...
	// Draw a cell
	void TableBase::_redraw_cell(TableContext context, int r, int c) {
		if (r < 0 || c < 0) return;
		int X, Y, W, H;
	...
	*/
	void TableBase::get_selection(int& row_top, int& col_left, int& row_bot, int& col_right) {
		if (select_col > current_col) {
	...
	*/
	void TableBase::set_selection(int row_top, int col_left, int row_bot, int col_right) {
		damage_zone(current_row, current_col, select_row, select_col);
	...
	//
		GraphicDrawHandle::PushClip(wix, wiy, wiw, wih);
		{
			Fl_Group::draw();
		}
		fl_pop_clip();
		...
		//{
			...
			if (!(damage() & FL_DAMAGE_ALL) && _redraw_leftcol != -1) {
				//GraphicDrawHandle::PushClip(tix, tiy, tiw, tih);
